...
**官方 CLI 0.2.50 自带的 `QUALITY_LABELS` 已整档错位**（缺 `gold` 档、把 `blue` 标成「稀有」），
不要拿它做参照。`orange` 在装备里没有样本，只见于技能书与遗迹碎片，
按 CLI 的档位顺序（在 `purple` 与 `red` 之间）定为「史诗」。映射的唯一来源是 `src/labels.mjs`，
经 `/labels.js` 路由同时供页面使用，前后端不会各写一份。
