...
- **三类首领分三个面板，各自设难度与名单。** 接口 `bossesByType` 实测返回个人 9 个、地图 5 个、世界 7 个，页面按服务端返回动态分组，不写死数量。
- **游戏里「地图首领」那一栏是 12 个 = 接口的 `type=map` 5 个 + `type=world` 7 个。** 玩法上地图首领和世界首领是分开的两回事，规则也分开：那 7 个在地图首领这一栏是**挑战**目标（带难度、胜率闸门、门票，由 `blockedReason` 判闸，受 `boss.difficulty` / `boss.mapBosses` 约束），在世界首领那一栏是**协作**目标（没有难度也不预测胜率，由 `assistBlockedReason` 判闸，受 `boss.worldBosses` / `boss.worldWindows` 约束）。同一个首领两种玩法各走各的规则，互不影响。实测这 7 个的 `blockedReason` 为空、`attempts` 为 1、`difficulties` 三档齐全且票价 0/1/2，与 `type=map` 的那 5 个完全同构。所以 `boss.map` 动作传的是 `[MAP, WORLD]` 两种类型 —— 早先只传 `[MAP]`，12 个里有 7 个从来没被挑战过。
