...
- **三类首领分三个面板，各自设难度与名单。** 接口 `bossesByType` 实测返回个人 9 个、地图 5 个、世界 7 个，页面按服务端返回动态分组，不写死数量。**地图这一类有出入待查**：游戏里的「地图首领」是独立一栏且有 12 个，而接口只给出 5 个（`nightmare_treant` / `demon_archon` / `eclipse_sovereign` / `frostfire_sovereign` / `chronos_sovereign`，分别对应黑暗森林、恶魔神殿、星渊圣域、霜火天穹、时墟神庭五张 50 级以上地图）。缺的那几个用什么键、走哪个端点还没查清，所以现在的地图首领自动化只覆盖接口给出的这 5 个。
