...
// 服务端后来插入 gold 档使中文名整体下移一位。**0.2.65 仍未修正**(已逐版核对到 0.2.65),
// 不要拿 CLI 那份当权威 —— 唯一来源就是下面这张表。
...
//
// **词要够具体,别用会出现在真失败里的泛词。** 实测踩过两次(2026-09-20,扫全部历史
// result_json 的 12 种错误消息):
//   ① 原来的裸词「不足」命中了「世界首领场次已变化、门票不足或攻击失败。」——
//      那是真失败(历史出现 14 次,其中一轮 7 个首领全挂),却被当成每日上限,
//      任务照样显示 ok。已删除:它唯一匹配到的就是这条真失败,没有任何良性消息靠它。
//   ② 「当前没有可领取的公会分红。」是"无事可做"而不是故障,原先没被收录,
//      于是公会任务只因当天没分红就显示 partial。已补「没有可领取」。
// 同一次统计里「上限」也一并删了:它被「已达上限」完全覆盖,留着只会扩大误判面。
export const BENIGN_ERROR_SOURCE =
  "(已领取|已用尽|已完成|完成捐献后|未开放|尚未开放|已达上限|最多|已参与|冷却|没有可领取)";
